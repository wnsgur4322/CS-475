...
#define NUM_ELEMENTS		32768       // signal.txt 
...
main( int argc, char *argv[ ] ){
    ...
    // file read
    FILE *fp_signal = fopen( "signal.txt", "r" );
    if( fp_signal == NULL )
    {
    ...
    fprintf( stderr, "%d\t%d\t%d\t%10.3lf MegaMults/Sec\n",
	...
	FILE* shift_res = fopen("cl_sums_result.txt", "a");
	for (int i = 1; i < 513; i++){
	...
	// write the result.txt file to record performance
	FILE* res;
	res = fopen("cl_result.txt", "a");
	
	fprintf(res, "%d\t%d\t%d\t%10.3lf\n", NUM_ELEMENTS, LOCAL_SIZE, NUM_WORK_GROUPS, (double)(Size * Size) / (time1 - time0) / 1000000. );
	fclose(res);
